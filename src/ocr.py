...
class SlideOCR:
    ...
    def extract_text_from_slides(
        ...
        for slide in aligned_slides:
            ...
            try:
                ...
                slide_copy["ocr_confidence"] = self._calculate_confidence(
                    ocr_text
                )

                logger.debug(
                    f"OCR extracted {len(ocr_text)} characters from slide {slide.get('slide_number')}"
                )

            except Exception as e:
                logger.warning(
                    f"OCR failed for slide {slide.get('slide_number')}: {e}"
                )
                slide_copy["ocr_text"] = ""
        ...
        logger.info(
            f"Completed OCR processing for {len(slides_with_ocr)} slides"
        )
    ...
    def extract_text_from_image(self, image_path: str) -> str:
        ...
        try:
            ...
            text = pytesseract.image_to_string(
                processed_image, config=ocr_config
            )
    ...
    def get_text_regions(self, image_path: str) -> List[Dict[str, Any]]:
        try:
            ...
            logger.error(
                f"Failed to extract text regions from {image_path}: {e}"
            )
            return []
    ...
    def filter_slides_by_text_content(
        ...
        for slide in slides_with_ocr:
            ...
            if (
                len(ocr_text) >= min_text_length
                and ocr_confidence >= min_confidence
            ):
        ...
        logger.info(
            f"OCR filtering: {len(slides_with_ocr)} -> {len(filtered_slides)} slides"
        )
        return filtered_slides

    def combine_transcript_and_ocr(
        self, slides_with_ocr: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        combined_slides = []
