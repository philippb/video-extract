...
class TranscriptDownloader:
    ...
    def download_transcript(self, video_id: str, language: str = None) -> List[Dict[str, Any]]:
    ...
    def _try_official_transcript(self, video_id: str, language: str) -> Optional[List[Dict[str, Any]]]:
        try:
            ...
            logger.debug(f"Raw transcript type: {type(raw_transcript)}, length: {len(raw_transcript)}")
    ...
    def _try_ytdlp_transcript(self, video_id: str, language: str) -> Optional[List[Dict[str, Any]]]:
        try:
            ydl_opts = {
                'writesubtitles': True,
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                available_subs = subtitles.get(language) or auto_captions.get(language)
    ...
    def _normalize_transcript(self, raw_transcript: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        normalized = []
        for entry in raw_transcript:
            ...
            normalized.append({
                ...
                'end': entry_dict.get('start', 0.0) + entry_dict.get('duration', 0.0),
                'text': entry_dict.get('text', '').strip()
            })
        return normalized
    
    def save_transcript(self, transcript: List[Dict[str, Any]], video_id: str) -> str:
