...
class FFmpegWrapper:
    ...
    def extract_frames_at_times(
        ...
        for i, timestamp in enumerate(timestamps):
            output_path = os.path.join(
                output_dir, f"slide_{i:04d}_{timestamp:.2f}s.png"
            )
            ...
            try:
                ...
                if result.returncode == 0 and os.path.exists(output_path):
                    ...
                    logger.warning(
                        f"Failed to extract frame at {timestamp:.2f}s: "
                        f"{result.stderr}"
                    )

            except subprocess.TimeoutExpired:
                logger.warning(
                    f"Frame extraction timed out at {timestamp:.2f}s"
                )
                continue
    ...
    def extract_uniform_frames(
        self, video_path: str, output_dir: str, interval: float = 30.0
    ) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Extracting frames at {interval}s intervals for "
