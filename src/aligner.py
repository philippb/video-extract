...
class TranscriptSlideAligner:
    ...
    def align_transcript_with_slides(
        self, transcript: List[Dict[str, Any]], slides: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        ...
        for i, slide in enumerate(slides_sorted):
            ...
            transcript_chunk = self._get_transcript_chunk(
    ...
    def _get_transcript_chunk(
        ...
        for entry in transcript:
            ...
            if self._overlaps_with_timerange(
                entry_start, entry_end, start_time, end_time
            ):
                chunk.append(entry)
    ...
    def _format_transcript_text(
        self, transcript_chunk: List[Dict[str, Any]]
    ) -> str:
    ...
    def _calculate_chunk_duration(
        ...
        end_time = max(
            entry.get("end", entry.get("start", 0.0))
            for entry in transcript_chunk
        )
    ...
    def get_slide_context(
        ...
        if (
            not aligned_slides
            or slide_index < 0
            or slide_index >= len(aligned_slides)
        ):
            return {}

        current_slide = aligned_slides[slide_index]
        context = {"current": current_slide, "previous": [], "next": []}
    ...
    def filter_slides_by_content(
        ...
        logger.info(
            f"Content filtering: {len(aligned_slides)} -> {len(filtered_slides)} slides"
        )
        return filtered_slides

    def merge_short_segments(
        self, aligned_slides: List[Dict[str, Any]], min_duration: float = 10.0
    ) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Segment merging: {len(aligned_slides)} -> {len(merged_slides)} slides"
        )
        return merged_slides

    def _merge_slides(
        self, slide1: Dict[str, Any], slide2: Dict[str, Any]
    ) -> Dict[str, Any]:
        merged = slide1.copy()

        merged["transcript_chunk"].extend(slide2.get("transcript_chunk", []))
        merged["transcript_text"] = self._format_transcript_text(
            merged["transcript_chunk"]
        )
        merged["duration"] = self._calculate_chunk_duration(
            merged["transcript_chunk"]
        )
    ...
    def get_alignment_stats(
        ...
        total_words = sum(
            slide.get("word_count", 0) for slide in aligned_slides
        )
        total_duration = sum(
            slide.get("duration", 0.0) for slide in aligned_slides
        )
        ...
        stats = {
            ...
            "avg_duration_per_slide": (
                total_duration / total_slides if total_slides > 0 else 0
            ),
            "avg_words_per_slide": (
                total_words / total_slides if total_slides > 0 else 0
            ),
