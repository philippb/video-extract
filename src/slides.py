...
class SlideExtractor:
    ...
    def extract_slides(
        self, video_id: str, scene_threshold: float = None
    ) -> List[Dict[str, Any]]:
        scene_threshold = scene_threshold or self.config.SCENE_THRESHOLD

        video_path = self._download_video(video_id)
        try:
            slides = self._extract_slides_from_video(
                video_path, video_id, scene_threshold
            )
            slides = self._remove_duplicates(slides)
            slides = self._filter_slides(slides)

            logger.info(
                f"Extracted {len(slides)} unique slides for video {video_id}"
            )
            return slides
    ...
    def _download_video(self, video_id: str) -> str:
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                raise RuntimeError(
                    f"Could not find downloaded video file for {video_id}"
                )
    ...
    def _extract_slides_from_video(
        self, video_path: str, video_id: str, scene_threshold: float
    ) -> List[Dict[str, Any]]:
        output_dir = self.config.get_slides_dir(video_id)

        try:
            scenes = self.ffmpeg.extract_scenes(
                video_path, output_dir, scene_threshold
            )

            if not scenes:
                logger.warning(
                    "No scene changes detected, using uniform sampling"
                )
            ...
            if len(timestamps) > self.config.MAX_SLIDES:
                logger.warning(
                    f"Too many slides detected ({len(timestamps)}), "
                    f"limiting to {self.config.MAX_SLIDES}"
                )
                timestamps = timestamps[: self.config.MAX_SLIDES]

            frames = self.ffmpeg.extract_frames_at_times(
    ...
    def _extract_uniform_slides(
        ...
        frames = self.ffmpeg.extract_uniform_frames(
    ...
    def _remove_duplicates(
        ...
        for slide in slides:
            try:
                ...
                logger.warning(
                    f"Error processing slide {slide.get('image_path')}: {e}"
                )
                continue

        logger.info(
            f"Removed {len(slides) - len(unique_slides)} duplicate slides"
        )
            ...
            try:
                ...
                with Image.open(image_path) as img:
                    if self._is_valid_slide(img):
                        ...
                        os.unlink(image_path)
                        logger.debug(
                            f"Filtered out invalid slide: {image_path}"
                        )

            except Exception as e:
                logger.warning(
                    f"Error filtering slide {slide.get('image_path')}: {e}"
                )
                continue

        logger.info(
            f"Filtered slides: {len(slides)} -> {len(filtered_slides)}"
        )
        ...
        for filename in sorted(os.listdir(slides_dir)):
            if filename.endswith(".png"):
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not parse timestamp from filename {filename}: {e}"
                    )
                    continue
