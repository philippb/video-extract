...
Examples:
  ...
  python cli.py VIDEO_ID --language es --max-slides 20
    ...
    parser.add_argument(
        "--max-slides",
        type=int,
        help="Maximum number of slides to extract"
    )
...
def main():
    try:
        ...
        log_file = "debug.log"
        logger = setup_logger("youtube-slide-summarizer", config.LOG_LEVEL, log_file)
        ...
        try:
            slides = slide_extractor.extract_slides(video_id, config.SCENE_THRESHOLD)
            if not slides:
        ...
        try:
            ...
            aligned_slides = aligner.filter_slides_by_content(aligned_slides, min_words=3)
            ...
            logger.info(f"Alignment stats: {stats['total_slides']} slides, "
        ...
        if args.dry_run:
            ...
            batch_summary = summarizer_temp.create_batch_summary(summarized_slides)
        else:
            logger.info("Step 5: Generating summaries...")
            try:
                use_vision = not args.no_vision
                summarized_slides = summarizer.summarize_slides(aligned_slides, use_vision)
                batch_summary = summarizer.create_batch_summary(summarized_slides)
        ...
        try:
            ...
            logger.info(f"File size: {output_summary.get('file_size_mb', 0):.2f} MB")
        ...
        print(f"\n❌ Unexpected error: {e}")
        if "--log-level" in sys.argv or "DEBUG" in os.environ.get("LOG_LEVEL", ""):
            import traceback
            traceback.print_exc()
        return 1
